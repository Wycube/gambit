...
void MAAudioDevice::pushSample(float left, float right) {
    m_samples_l.push(left);
    m_samples_r.push(right);
}

auto MAAudioDevice::full() -> bool {
    return m_samples_l.size() == m_samples_l.capacity();
}

void MAAudioDevice::setSampleRate(int resolution) {
    m_samples.store(512 << resolution);
    m_sample_counter.store(0);
    LOG_INFO("Sampling rate {}KHz selected", 32 << resolution);
}

void MAAudioDevice::resample(float *dst, size_t channel_size) {
    size_t frame_size = m_samples.load();
    std::vector<float> frame(frame_size * 2);

    m_samples_l.pop_many(frame.data(), frame_size);
    m_samples_r.pop_many(frame.data() + frame_size, frame_size);
...
void MAAudioDevice::audioCallback(ma_device *device, void *output, const void *input, ma_uint32 frame_count) {
    ...
    float samples[2048];
    audio_device->m_samples_l.pop_many(samples, 1024);
    audio_device->m_samples_r.pop_many(samples + 1024, 1024);
    // frontend->m_audio_buffer_mutex.lock();
