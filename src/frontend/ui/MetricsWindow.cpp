...
void MetricsWindow::setCPUUsage(const float *times) {
    std::memcpy(cpu_usage, times, sizeof(cpu_usage));
}

void MetricsWindow::draw() {
    if(ImGui::Begin(name, &active)) {
        ImVec2 plot_size = ImGui::GetContentRegionAvail();
        plot_size.y = std::min(100.0f, plot_size.y / 3.0f) - ImGui::GetStyle().FramePadding.y;
        ...
        ImGui::PlotHistogram("##Emulator_CPU_Usage", cpu_usage, 100, 0, "CPU Usage", 0.0f, 100.0f, plot_size);
    }
    ImGui::End();
}